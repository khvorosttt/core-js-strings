...
function isPalindrome(str) {
  const newStr = str.toLocaleLowerCase().replace(/[^\w]/g, '');
  const center = newStr.length / 2;
...
function encodeToRot13(str) {
  ...
  const lettersRot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str
    .split('')
    .map((letter) => {
      if (letters.includes(letter)) {
        return lettersRot13.charAt(letters.indexOf(letter));
      }
      return letter;
    }, '')
    .join('');
